...
const Documentation = () => {
    const sections = [
        ...
        },
        {
            ...
            icon: Server,
            content: "Quantum Vault can integrate with hardware security modules. A hardware security module, often referred to as an HSM, is a specialized device designed to securely generate and store cryptographic keys. Unlike software based key storage, an HSM keeps keys inside a protected hardware environment that is designed to resist tampering and unauthorized access. When cryptographic operations are performed through an HSM, the private keys remain inside the device while the required cryptographic computation takes place. This reduces the likelihood that keys will be exposed to application level systems or external attackers."
        },
        {
            id: "key-management",
        ...
        },
        {
            ...
            title: "Audit Logging",
            icon: ClipboardList,
            content: "Quantum Vault maintains audit logs that record important cryptographic activities within the system. These logs provide visibility into how cryptographic resources are used across applications and services. Audit records may include information about key creation events, cryptographic operations such as encryption or signing, authentication attempts, and updates to access policies. Maintaining detailed logs helps organizations review system behavior and detect unusual patterns that may indicate security concerns."
        },
        {
            id: "architecture",
                            ...
                            <nav className="space-y-1">
                                {sections.map((section) => (
                                    <a
                                        key={section.id}
                                        href={`#${section.id}`}
                                        className="block px-4 py-2 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-brand-primary dark:hover:text-brand-primary transition-colors border-l-2 border-transparent hover:border-brand-primary"
                                    >
                                        {section.title}
                                    </a>
                                ))}
                        ...
                        <div className="lg:col-span-9 space-y-20">
                            ...
                            >
                                ...
                                <div className="relative z-10 text-center max-w-3xl mx-auto">
                                    ...
                                    <p className="text-xl md:text-2xl text-slate-300 dark:text-slate-400 font-medium leading-relaxed mb-10">
